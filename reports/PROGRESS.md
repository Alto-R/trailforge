...
**日期**：2026-06-23　**仓库**：[Alto-R/trailforge](https://github.com/Alto-R/trailforge)　**范围**：北京登山（climbing）单活动
...
**研究主线（混合 CF）已闭环并得到决定性验证；工程主线后端 `/route` + 前端 demo 全栈跑通；T2.6 路径生成器已补环线+局部搜索。算法/工程能由代码完成的部分基本全部就位，剩下的是需要线下组织的 T3.5 真人用户测试。**
数据资产、用户/片段表示、混合 CF 模型与完整对照实验（含冷启动）全部就位（[T1.1](T1.1_backend_recommender.md) 服务化）。**决定性工程发现：E4 内容塔不个性化（§2.4），据此按主计划 §4.5 转探索辅助形态**——多候选 + 显式偏好排序 + MMR 多样性 + 可解释标签 + 反馈。前端 demo（[T1.6](T1.6_frontend.md)）已交付。**本轮（2026-06-23）补齐 T2.6 环线（方向惯性+Dijkstra 保底，回到起点、可出真环线）+ 局部搜索，并端到端接到 `/route?loop` 与前端复选框（[T2.6](T2.6_route_loop_localsearch.md)）；全量测试 49 项全绿（40 Python + 9 前端）。**
...
| **Phase 2** 算法核心 | 用户表示/CF/评估 | ✅ ~100% | 用户表示、片段表示、混合 CF、E0–E6 矩阵、冷启动、5-seed 显著性+冷启动消融(T2.7) 全部完成；**路径生成器 CF 化+环线+局部搜索(T2.6) ✅**（环线回到起点、可出真环线，前后端+5 测试绿）→ [T2.6](T2.6_route_loop_localsearch.md) |
| **Phase 3** 整合测试 | 集成/用户测试 | 🟡 ~15% | T3.1/T3.2 多样性+可解释+集成已随 T1.1/T1.6 落地；T3.4 端到端测试覆盖 49 项全绿；**T3.5 小规模真人用户测试需线下组织（不可由代码完成）** |

> **关键判断**：研究目标已达成（核心算法验证完毕、有可发表结论）；产品目标（demo）已可演示——后端 `/route`（开放/环线）+ 前端探索辅助界面全栈跑通，T2.6 已闭环。**剩余唯一计划项是 T3.5 小规模真人用户测试，需导师线下组织招募，无法由代码完成。**
...
| T2.6 环线+局部搜索 | T2.6 | [T2.6_route_loop_localsearch.md](T2.6_route_loop_localsearch.md) | `generate_loop`（方向惯性+Dijkstra保底，回到起点、可出真环线）+ `local_search`；`/route?loop=true` + 前端复选框；+5 测试 |
...
- **范围**：仅北京登山单活动单分量；徒步/骑行未展开（需 G: 盘原始数据，当前未挂载）。
- **路径生成**：T2.6 已补环线（方向惯性+Dijkstra 保底，回到起点、可出真环线）+ 局部搜索；但**环线常短于预算**（最短回程压缩周长，内在特性，已用 reachable/note 如实告知），局部搜索在近线性网络增益≈0。详见 [T2.6](T2.6_route_loop_localsearch.md)。
...
| ✅ 完成 | **T1.6 前端**：地图起点点选 + 偏好滑块/persona + 候选列表 + 高亮 + 反馈 + **环线复选框** | demo 可演示 |
| ✅ 完成 | **T2.6 路径生成 环线 + 局部搜索**（CF 化已在 T1.1 完成）→ [T2.6](T2.6_route_loop_localsearch.md) | 环线回到起点、可出真环线；CF 仅作先验（内容塔不个性化，故不做按用户 CF 路线打分） |
| ✅ 完成 | onboarding 冷启动映射（[persona.py](../src/persona.py)：GMM 聚类→冷用户 u_feat + 数据派生默认偏好）+ 可解释标签 | 产品形态完整；u_LLM/视觉作可选解释而非主信号 |
| **高（不可代码完成）** | **T3.5 小规模真人用户测试**（5–10 位驴友，onboarding→自由探索→任务→问卷） | 需导师线下组织招募；代码侧已就绪 |
| 中 | 等长回程的严格环线 / 徒步·骑行扩展（需挂载 G: 盘原始数据） | 提升环线顶预算能力 / 扩活动域 |
