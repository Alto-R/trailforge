...
数据资产、用户/片段表示、混合 CF 模型与完整对照实验（含冷启动）全部就位。后端把成果做成 FastAPI 服务（[T1.1](T1.1_backend_recommender.md)）。**过程中得到一个决定性工程发现：E4 内容塔不个性化（见 §2.4），据此按主计划 §4.5 转为探索辅助形态**——多候选 + 显式偏好排序 + MMR 多样性 + 可解释标签 + 反馈。距"可演示 demo"只差前端。
...
| **Phase 1** 基础+数据 | 后端重构/数据资产/规则 demo | 🟡 ~75% | 数据资产 + 路径生成器 ✅；**后端 `/route` 服务(T1.1) ✅（探索辅助，27 测试绿）**；**前端(T1.6) 未起** |
