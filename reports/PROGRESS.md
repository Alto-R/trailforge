...
**日期**：2026-05-30　**仓库**：[Alto-R/trailforge](https://github.com/Alto-R/trailforge)　**范围**：北京登山（climbing）单活动
...
**研究主线（混合 CF）已基本闭环并得到决定性验证；工程主线（后端 API / 前端 demo）尚未启动。**
数据资产、用户/片段表示、混合 CF 模型与完整对照实验（含冷启动）全部就位；规则版路径生成器可跑。距"可演示 demo"只差后端服务化 + 前端交互。
...
| **Phase 1** 基础+数据 | 后端重构/数据资产/规则 demo | 🟡 ~55% | 数据资产 + 路径生成器 ✅；**后端 API(T1.1) / 前端(T1.6) 未起** |
| **Phase 2** 算法核心 | 用户表示/CF/评估 | ✅ ~90% | 用户表示、片段表示、混合 CF、E0–E6 矩阵、冷启动全部完成；路径生成器 CF 化+环线(T2.6)待做 |
| **Phase 3** 整合测试 | 集成/用户测试 | ⬜ 0% | 依赖工程主线 |

> **关键判断**：研究目标已接近达成（核心算法验证完毕、有可发表结论）；产品目标（demo）卡在尚未开工的前后端服务化。
...
3. **内容侧组件在活跃域 ROI≈0**：LLM 画像(E3≈E5)、彭晓视觉(E6≈E3) 边际增益≈0——但其价值在冷启动（内容塔整体在冷启动决定性有用）。单看活跃域会误判"砍内容"，冷启动纠正之。
...
- **评估**：单次运行，未做多 seed 显著性；负样本为地理邻近（相对易）。
...
| 优先级 | 任务 | 价值 |
|---|---|---|
| 高 | 多 seed(≥5) 显著性 + 冷启动域内部消融(去 u_LLM/视觉) | 论文级；定位冷启动里哪层内容真有用 |
| 高 | **工程线**：T1.1 后端 `/route` API（E4 模型 + T1.5 生成器）+ T1.6 前端起点点选 | 把研究成果变成可演示 demo |
| 中 | T2.6 路径生成 CF 化 + 环线 + 局部搜索 | 推荐路线接入个性化评分 |
| 中 | onboarding 冷启动映射 + 可解释标签（用 u_LLM/视觉的真实用途） | 产品形态完整 |
